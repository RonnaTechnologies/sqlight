...
public:
  ...
  void exec(std::string_view query) {
    char *err = nullptr;
    sqlite3_exec(db_ptr, query.data(), nullptr, nullptr, &err);
  }

  template <typename... Args, template <typename...> typename Query_t,
            typename... QueryArgs>
    requires std::same_as<Query_t<QueryArgs...>, query<QueryArgs...>>
  std::vector<std::tuple<Args...>>
  execute(const Query_t<QueryArgs...> &sql_query) {

    sqlite3_stmt *statement = nullptr;
    sqlite3_prepare_v2(db_ptr, sql_query.query_string.data(),
                       static_cast<int>(sql_query.query_string.size()),
                       &statement, nullptr);

    [&]<auto... Is>(std::index_sequence<Is...>) {
      ((bind_param(statement, std::get<Is>(sql_query.query_args), Is + 1)),
       ...);
    ...
    while (step == SQLITE_ROW) {
      ...
      [&]<auto... Is>(std::index_sequence<Is...>) {
        ((get_column(statement, Is, std::get<Is>(row))), ...);
